...
class ServicesRequest: ObservableObject {
    ...
    func requestServices(tlaref: String) async throws ->  FormattedServices {
        guard let url = URL(string: "https://api.sandbox.livetramsmcr.com/v1/services/\(tlaref)") else {
            print("Invalid url...")
    ...
    func requestDepartureBoardServices(tlaref: String) async throws ->  DepartureBoardServices {
        guard let url = URL(string: "https://api.sandbox.livetramsmcr.com/v1/services/departure-boards/\(tlaref)") else {
            print("Invalid url...")
